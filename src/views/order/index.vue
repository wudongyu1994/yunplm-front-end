<template>
  <div class="app-container">
    ...
    <!--订单编辑对话框-->
    <el-dialog title="Edit Order" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="orderRules" :model="temp" label-position="right" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="expressNo" prop="expressNo">
          <el-input v-model="temp.expressNo" clearable />
        </el-form-item>
    ...
    <el-dialog :title="textMap[dialogStatus] + 'Material'" :visible.sync="materialModifyVisible">
      <el-form ref="materialForm" :rules="rulesMaterial" :model="tempMaterial" label-position="right" label-width="180px" style="width: 400px; margin-left:50px;">
        <el-form-item label="id" prop="id">
        ...
        <el-form-item label="material name" prop="name">
          <el-input v-model="tempMaterial.name" clearable />
        </el-form-item>
        <el-form-item label="corp" prop="corp">
          <el-input v-model="tempMaterial.corp" clearable />
        </el-form-item>
        <el-form-item label="batchNo" prop="batchNo">
          <el-input v-model="tempMaterial.batchNo" clearable />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
import {
  getOrderByPage,
  getProductItemByOrderId,
  modifyOrder,
  deleteOrder,
  getOrderByUserIdByPage
} from '../../api/order'
...
import { isNo, isPhone } from '../../utils/validate'
import { getLogisticsByExpressNo } from '../../api/logistics'
...
export default {
  ...
  data() {
    const validateNoChinese = (rule, value, callback) => {
      if (value.length && isNo(value)) {
        callback()
      } else {
        callback(new Error('limit: a-z,A-Z,0-9'))
      }
    }
    ...
    return {
      ...
      dialogStatus: '',
      orderRules: {
        expressNo: [{ validator: validateNoChinese, trigger: 'blur' }],
        phone: [{ validator: validatePhone, trigger: 'blur' }]
      },
      rulesMaterial: {
        name: [{ required: true, message: 'name is required', trigger: 'blur' }],
        corp: [{ required: true, message: 'corp is required', trigger: 'blur' }],
        batchNo: [{ validator: validateNoChinese, trigger: 'blur' }]
      }
    }
  },
  computed: {
  ...
  methods: {
    getOrder() {
      ...
      if (this.authorities.some(val => val === '/order/admin')) {
        ...
        getOrderByUserIdByPage(this.principal.id, this.pageSetting).then(response => {
          this.orderListRaw = response.data
          this.total = response.count
          this.listLoading = false
        })
    ...
    handleShowLogistics(row) {
      ...
      getLogisticsByExpressNo(row.expressNo).then(response => {
