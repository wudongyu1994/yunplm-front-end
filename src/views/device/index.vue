    ...
    <el-dialog title="add device" :visible.sync="addDeviceDialogVisible">
      <el-form ref="dataForm" :rules="addDeviceDialogRules" :model="temp" label-position="right" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="productKey" prop="productKey">
          <el-input v-model="temp.productKey" clearable />
        </el-form-item>
        <el-form-item label="deviceName" prop="deviceName">
          <el-input v-model="temp.deviceName" clearable />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getDevice() {
      ...
      if (this.authorities.some(val => val === '/device/admin')) {
        getAllDevice().then(response => {
          this.deviceList = response.data
          this.listLoading = false
        })
      } else {
        getDeviceByUserId(this.principal.id).then(response => {
          this.deviceList = response.data
          this.listLoading = false
        })
      }
